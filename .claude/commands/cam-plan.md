...
- Read the resolved issue's full spec (already fetched in Step 2): GitHub via `gh issue view N --json title,body,labels,comments,state,url`, the local per-file issue entry for `none`, or the Linear API result.
