...
**CLI path**: `gship ship` (run from a terminal outside the session) writes `phase:shipping` to `.claude/cam-loop.local.md` and returns immediately. The sidecar detects `phase === 'shipping'` on its next tick and calls `runShipPhase` (`src/supervisor/ship-runner.ts`).

**Slash-command path** (this file): write the same `phase:shipping` signal, then narrate. Both paths are signal-writers only; the full control-flow (branch guard, PRD-complete gate, quality gates, version bump, cycle-close finalize, push, PR-create) lives entirely in `runShipPhase` and `runShipPrStep` (`src/release/ship-pr.ts`). No LLM participates in the ship path: the PR title and body are composed deterministically from the PRD snapshot (`composePrTitle`/`composePrBody`, `src/release/pr-body.ts`). The PR body template renders five fixed sections in order: Summary, Stories completed, Testing, Notes, and Setup checklist (a `| # | Item | Where | How | Status |` table of manual user actions the branch requires, e.g. new env vars, installs, migrations, credentials, webhooks, detected from cues in the PRD description/notes/story titles; a clear empty-state line renders instead of an empty table when nothing is detected). The decision to move this control-flow out of markdown is recorded in `docs/adr/0009-ship-phase-deterministic-sidecar-runner.md` (see also `docs/adr/0004-plan-phase-deterministic-sidecar-runner.md` and `docs/adr/0006-phase-enum-loop-state-machine.md`).
