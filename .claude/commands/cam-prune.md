...
`gship prune` (US-004, CAM-400) is a deterministic CLI subcommand — no LLM judgment
...
On success, `gship prune` prints the pruned branch name and confirms the
working tree is on main — relay that to the user.

If `gship prune` exits nonzero, read its printed error and act on it:
...
Rules:
- NEVER delete `main` or `master` — `gship prune` enforces this internally and
