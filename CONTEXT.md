...
**doc-as-code gate**:
A retired legacy gate that checked factual claims inside the old agent-persona documentation. It was removed with the persona/template bundle; current product documentation is kept small and validated by the runtime and focused tests it describes.

**known-missing allowlist**:
The historical allowlist used by the retired doc-as-code gate for runtime-only paths. It no longer participates in the live gate spine.
