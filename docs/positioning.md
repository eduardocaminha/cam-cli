...
The product name is **Gateship**, decided 2026-07-27 and recorded in ADR-0050. ADR-0050 also formalizes a fourth name-gate layer, a live homonymous product occupying the category, distinct from domain/npm/GitHub-org availability, that an earlier, since-superseded naming decision failed to clear. The two ADRs (`docs/adr/`) hold the full multi-round research process, the kill evidence for every candidate considered, the four-layer gate, and the ranked reserves as the historical record; this section states the current decision only and does not re-narrate that history.
