...
- Schema JSON canonico em `scripts/cam/issues.schema.json` e `templates/scripts/cam/issues.schema.json` (byte-identicos). O schema descreve um unico `IssueEntry` (nao o array legado do arquivo monolitico).
- Script de migracao idempotente (`scripts/cam/migrate-issues-schema.ts`): issues fechadas para `shipped`, abertas para `idea`. Executado em producao no CAM-90 US-005: o arquivo monolitico legado foi deletado e substituido por `scripts/cam/issues/CAM-NNNN.json` (um arquivo JSON por issue, 4 digitos com padding, campo `id` sem padding). A documentacao operacional foi atualizada no CAM-90 US-008.
