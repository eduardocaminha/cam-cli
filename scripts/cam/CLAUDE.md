...
**Recorte of the `claude -p` prohibition (ADR-0059, CAM-516):** the invariant above is recortada, not aposentada. It stays fully prohibited for the tmux worker path: the five test files (`test/supervisor/loop.test.ts`, `test/supervisor/worker-argv.test.ts`, `test/supervisor/review.test.ts`, `test/supervisor/plan/plan-argv.test.ts`, `test/supervisor/docker-exec.test.ts`), `scripts/cam/patterns.md`'s bullet, and the comment in `src/commands/run-auth-preflight.ts` keep asserting no `-p`/`--print` reaches a tmux worker argv. The `gship next --headless` invocation is born exempt, opted into only by that explicit CLI flag, never by a persisted config key. The exemption rests on the 2026-08-08 billing measurement recorded in ADR-0059 (`docs/adr/0059-execucao-headless-do-worker-a-proibicao-de-2026-06-e-recortada-nao-aposentada.md`): a `--print`/stream-json call left `consumo de API inalterado` (console API consumption unchanged), the process reported `apiKeySource none`, and the response carried a `rate_limit_event` with a `sete dias` (seven-day) subscription window, measured with the worker `rodando no host e autenticando pelo diretorio de configuracao` (on the host, authenticating through its config directory). The container-plus-env-token combination was not measured and stays out of scope.
