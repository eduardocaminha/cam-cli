...
- **tmux window-resized hook + resize-pane -x clamp** (CAM-48, US-003, 2026-06-14): `resize-pane -x` rejects tmux format expressions like `#{window_width}` with "width invalid" - the value must be a pre-computed literal integer. tmux comparison modifiers (`#{>:a,b}`) are lexical (string), not numeric, so a numeric min/max clamp cannot be expressed in pure tmux format. Solution: use `run-shell 'w=#{window_width}; t=$(( w*20/100 )); t=$(( t<34?34:t>52?52:t )); tmux resize-pane -t <paneId> -x $t || true'`. `run-shell` causes tmux to expand `#{window_width}` to a numeric literal before passing the string to `sh -c`, enabling shell `$(())` arithmetic for the clamp. One `resize-pane` on the dashboard pane id re-clamps the entire right column (menu pane shares the column width). Verified on tmux 3.6a: w=100->34, w=300->52, w=220->44.
