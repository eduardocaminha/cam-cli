...
- **tmux window-resized hook + resize-pane -x clamp** (CAM-48, US-003 + US-R1-001, 2026-06-14): `resize-pane -x` rejects tmux format expressions like `#{window_width}` with "width invalid" - the value must be a pre-computed literal integer. tmux comparison modifiers (`#{>:a,b}`) are lexical (string), not numeric, so a numeric min/max clamp cannot be expressed in pure tmux format. Solution: use `run-shell 'w=#{window_width}; t=$(( (w*20+50)/100 )); t=$(( t<34?34:t>52?52:t )); tmux -L <socket> resize-pane -t <paneId> -x $t || true'`. `run-shell` causes tmux to expand `#{window_width}` to a numeric literal before passing the string to `sh -c`, enabling shell `$(())` arithmetic for the clamp. Use `(w*20+50)/100` (round-half-up) not `w*20/100` (truncation) to match `Math.round` in the JS helper. One `resize-pane` on the dashboard pane id re-clamps the entire right column (menu pane shares the column width). Interpolate the socket constant (CAM_TMUX_SOCKET) in the TS template string instead of a hardcoded literal. Verified on tmux 3.6a: w=100->34, w=188->38, w=220->44, w=400->52.
