...
//
// Implementation of `cam plan` -- thin-proxy that routes /cam-plan to the
// live orchestrator pane via send-keys (US-006).
//
// Acceptance criteria (US-006):
//   1. Detect a live orchestrator via orchestratorAlive (US-005 predicate).
//   2. On hit: atomic send-keys /cam-plan [N] to the orchestrator pane and
...
/**
 * Run the `cam plan` flow: thin-proxy to the live orchestrator (US-006).
 *
 * If the orchestrator is already running (hasSession + orchestratorAlive):
 *   - Sends `/cam-plan` or `/cam-plan N` to the orchestrator pane via send-keys.
 *
...
export async function runPlan(options: PlanOptions = {}): Promise<number> {
	...
	const request = issue !== undefined ? `/cam-plan ${issue}` : '/cam-plan';
	...
	if (!alive) {
		...
		if (!bootstrapped) {
			printError(
				...
				'Run `cam run` manually, then retry `cam plan`.',
			);
			emitTrailingBlank();
			return 1;
		}
