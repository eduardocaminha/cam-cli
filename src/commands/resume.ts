...
//
// Reconciles the loop state file, PRD, supervisor PID, worker-pane marker and
// last commit into one recovery decision. Completed PRDs clean stale state;
// live processes are left alone; recent dead loops can respawn; old or unknown
// ones require an operator choice. Explicit reset modes remain available for
// story, PRD and branch rollback.
...
import { type LoopState, type PrdShape, parseStateFile, resolvePrdPath } from './status.ts';
...
/**
 * Heuristic cutoff for distinguishing a recent interruption from a hard-kill
 * orphan. 24h means an operator who paused their
...
export type ResumeMode =
	| 'live-supervisor' // supervisor PID is alive (already running)
	| 'respawn' // state file present, PID dead, recent activity
	| 'orphaned-worker' // no supervisor PID but worker slot pane is alive
	| 'prompt' // needs operator [Y/n/reset]
 ...
 *   else if last commit > 24h old (or null)       → prompt (hard-kill orphan)
 *   else                                          → respawn (recent crash)
 */
...
export function buildResumeReport(options: ResumeOptions = {}): ResumeReport {
	...
	const decision = classifyResumeMode(state, prd, lastCommitMs, pidAlive, now(), workerPaneAliveResult);
 ...
 *
 * Exit codes:
 *   0 — recovery complete OR `--dry-run` succeeded OR prompt → reset/continue
 *   1 — prompt → `n` (operator declined to recover); no work done
...
function printSummary(report: ResumeReport): void {
	...
	// prompt → warning yellow, idle → muted (these are quiet states),
