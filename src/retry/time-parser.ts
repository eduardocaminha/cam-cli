...
export interface AbsoluteParsedTime {
  hour: number;
  minute: number;
...
export interface RelativeParsedTime {
  relative: true;
  waitMs: number;
}

export type ParsedTime = AbsoluteParsedTime | RelativeParsedTime;

export function parseResetTime(text: string): ParsedTime | null {
  // Try absolute time first: "resets at 3pm (UTC)"
  const absMatch = text.match(RESET_TIME_REGEX);
  if (absMatch) {
    let hour = parseInt(absMatch[1] ?? '0', 10);
    const minute = absMatch[2] ? parseInt(absMatch[2], 10) : 0;
    const ampm = absMatch[3] != null ? absMatch[3].toLowerCase() : null;
...
export function calculateWaitMs(
  ...
  const abs = parsed as AbsoluteParsedTime;

  let tz: string;
  try {
    tz = abs.timezone ?? Intl.DateTimeFormat().resolvedOptions().timeZone;
  ...
  function getTargetTimestamp(h: number, m: number): number {
  ...
  if (abs.ambiguous) {
    const t1 = getTargetTimestamp(abs.hour, abs.minute);
    const t2 = getTargetTimestamp(abs.hour + 12, abs.minute);
  ...
  let diff = getTargetTimestamp(abs.hour, abs.minute) - now.getTime();
