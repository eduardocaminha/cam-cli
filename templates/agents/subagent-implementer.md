...
The SIDECAR (`runSupervisor`, a background process spawned by `gship run`) invokes you once per story. Do not loop, do not try to do two stories, do not decide when you're "done with the project": the sidecar owns scheduling.
...
3. Read `handoff.json` for the previous story's context. Treat `nextStoryContext` as advisory, not authoritative; `acceptanceCriteria` always wins on conflict.
...
2. Run quality gates: typecheck, lint, tests. Fix until green. Use the project's configured commands. Also run the full `bun run check:all` spine in-story after coding, not just typecheck+test+file-size — this fires every sibling ratchet (coverage, debt-markers, dead-code/knip, dup/jscpd) alongside file-size during the story run, so a legitimate change that trips one is caught and justified now instead of surfacing late at ship/CI as a manual raise. Resolve each failing gate inline per this rubric: **file-size** — if a file this story legitimately grew now exceeds its ceiling, raise ONLY that file's ceiling in `scripts/file-size-budget.json` to the gate's reported actual line count (measured as `content.split('\n').length`, explicitly NOT `wc -l`, which under-counts a trailing-newline file by one), prepend a dated note naming this story and the CAM tracker id to the top-level `_ref` field, and stage `scripts/file-size-budget.json` so the tracker-ref check (which reads only the staged diff) passes before re-running the gate — raise only the ceiling(s) of the file(s) this story itself grew, never blanket-raise ceilings for unrelated budgeted files. **coverage** — if this story's diff legitimately lowers measured coverage, lower the affected global floor(s) in `scripts/coverage-budget.json`, prepend a CAM tracker-ref to the top-level `_ref` field, and stage the file in the same commit — the same `_ref`/tracker mechanics as file-size, but over two global floors (functions, lines), not per-file. **debt-markers** — a new TODO/FIXME must cite an inline CAM tracker ref on the same line; this gate has no `_ref` file to raise. **dead-code (knip)** — remove the dead code, or add a justified ignore/exclude entry to `knip.json`; knip has NO `_ref` channel, so a suppression relies on the reviewer backstop, not a staged tracker-ref. **dup (jscpd)** — deduplicate, or bump the threshold / add an ignore entry in `.jscpd.json` with a justification in the commit message; jscpd also has NO `_ref` channel, the same reviewer-backstop reliance as knip.
