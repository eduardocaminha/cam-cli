...
describe('no `--permission-mode` flag on any subcommand parser (behavioral)', () => {
	...
	test('parseReviewArgs rejects --permission-mode', () => {
		const original = process.stderr.write.bind(process.stderr);
		const captured: string[] = [];
		process.stderr.write = ((chunk: string | Uint8Array) => {
			captured.push(typeof chunk === 'string' ? chunk : new TextDecoder().decode(chunk));
		...
		try {
			const result = parseReviewArgs(['--permission-mode', 'acceptEdits']);
			expect(result).toBeNull();
		} finally {
			process.stderr.write = original;
		}
	...
	test('parseShipArgs rejects --permission-mode', () => {
		...
		try {
			const result = parseShipArgs(['--permission-mode', 'acceptEdits']);
			expect(result).toBeNull();
		} finally {
			process.stderr.write = original;
		}
		expect(captured.join('')).toMatch(/unknown ship option.*--permission-mode/i);
	});
});
