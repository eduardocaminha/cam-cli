...
describe('runPlan attach hint', () => {
	test('emits attach hint when caller is detached (no TMUX)', async () => {
		...
		const readBacklogFn = () => [
			{ id: 'CAM-1', title: 'Test', stage: 'specified' as const, status: 'open' as const, blockedBy: [], createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z', spec: { acceptanceCriteria: ['x'], scope: 's', gotchas: [], domainTerms: [] } },
		];
		const writeFn = () => '';
