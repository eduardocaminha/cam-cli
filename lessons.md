...
## 2026-06-15: smoke operator achou 2 criticals que 3 rounds CLEAN + 925 testes nao pegaram (tmux real)

Situacao: CAM-55 (single-hub) passou nos 925 testes unitarios e em 3 rounds de review CLEAN. No smoke operator (US-FIX-008), rodei `cam run` + `cam next` de verdade contra tmux + claude reais. O `cam next` falhou primeiro com "No live orchestrator detected, bootstrapping" e depois com "Could not find orchestrator pane", apesar da sessao + pane + marker existirem.

Achado: dois bugs reais-de-tmux invisiveis aos fakes (os fakes codificavam a saida que o codigo ESPERAVA, batendo com o codigo bugado). (1) `orchestratorAlive` checava `pane_current_command === 'claude'`, mas o pane do orquestrador roda claude sob um wrapper bash de respawn (CAM-23), entao o comando e sempre `bash`: detecao tem que ser por `@cam_label`, nunca por comando. (2) tmux converte TAB (0x09) em `_` (0x5f) na saida de `-F` (`tmux display-message -p $'A\tB'` emite `A_B`): `getOrchPaneId` e `isSessionStale` (o proprio fix do C3) usavam TAB como separador e nunca casavam a saida real. Troca para `;`. Bonus: ~20 chamadas de `runRun` em run.test.ts nao injetavam `spawnSidecarFn`, spawnando `cam sidecar` reais que idle-loopam e travam `bun test` (latente ate o binario virar instalavel+current).

Correcao aplicada: orchestratorAlive label-based; getOrchPaneId + isSessionStale com separador `;`; noopSidecar em todos os runRun de teste; ~16 fakes migrados; teste de integracao real-tmux (`test/integration/tmux-introspect.test.ts`) que exercita os 3 helpers contra tmux real e teria pego ambos. Smoke re-rodado: C1/C2/C3/C4 + detecao + getOrchPaneId provados ao vivo. PR #54.

Regra (local canonico): scripts/cam/patterns.md (gotcha "tmux mangles TAB in -F output; key panes on @cam_label, not command"). Meta-licao reforca 2026-06-05 ("comentario/fake nao prova comportamento; verifique o runtime"): fake que retorna a saida esperada nao pega bug de comportamento-real da ferramenta; toda camada que parseia `tmux -F` precisa de um teste real-tmux.
